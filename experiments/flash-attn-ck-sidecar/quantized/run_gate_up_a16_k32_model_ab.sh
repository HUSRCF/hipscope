...
RESULT_DIR="${RESULT_DIR:-${QUANT_ROOT}/results/gate_up_a16_k32_model_ab_$(date +%Y%m%d_%H%M%S)}"
FEATURE_ENV="${FEATURE_ENV:-HIPFIRE_RDNA3_HFQ4_GATE_UP_A16_K32}"
FEATURE_ENVS="${FEATURE_ENVS:-${FEATURE_ENV}}"
...

{
    ...
    echo "trials=${TRIALS}"
    echo "candidate_envs=${FEATURE_ENVS}"
...
run_one() {
    ...
    if [[ "${mode}" == "candidate" ]]; then
        local feature
        for feature in ${FEATURE_ENVS}; do
            candidate_env+=("${feature}=1")
        done
    fi
    ...
    [[ "${ck_active}" == 1 ]] || {
        echo "quantized CK attention did not activate: ${log}" >&2
        return 3
    }

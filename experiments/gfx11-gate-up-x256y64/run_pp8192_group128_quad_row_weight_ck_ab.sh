...
OUT_DIR="${OUT_DIR:-${ROOT}/experiments/gfx11-gate-up-x256y64/results/pp8192_group128_quad_row_weight_ck_gpu1_${STAMP}}"
...
run_command() {
    ...
    timeout --signal=INT --kill-after=5s 240s env \
        ...
        HIPFIRE_RDNA3_Q8_GROUP128_QUAD_ROW_WEIGHT="${quad_row}" \
...
run_one() {
    local pair="$1" order="$2" mode="$3" quad_row=0
    [[ "${mode}" == quad_row ]] && quad_row=1
    local log="${OUT_DIR}/pair_${pair}_${order}_${mode}.log"
    run_command "${log}" "${quad_row}"
...
sleep "${SLEEP_SECS}"
run_command "${OUT_DIR}/prewarm_quad_row.log" 1
sleep "${SLEEP_SECS}"

for ((pair=1; pair<=TRIALS; pair++)); do
    if ((pair % 2)); then modes=(baseline quad_row); else modes=(quad_row baseline); fi
    for order in 0 1; do
...
quad_row = statistics.median(values("quad_row", "prefill_tok_s", True))
...
print(f"trim_each_side={trim} baseline_trimmed_median={baseline:.3f} "
      f"quad_row_trimmed_median={quad_row:.3f}")
print(f"quad_row_vs_baseline={quad_row / baseline:.4f}x "
      f"({(quad_row / baseline - 1) * 100:+.2f}%)")
print(f"token_ids_match={tokens['baseline'] == tokens['quad_row']} "
      f"baseline_variants={len(tokens['baseline'])} "
      f"quad_row_variants={len(tokens['quad_row'])}")
PY

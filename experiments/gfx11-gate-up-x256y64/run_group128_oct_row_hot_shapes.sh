...
OUT=${OUT:-$ROOT/experiments/gfx11-gate-up-x256y64/results/oct_row_weight_loader_gpu1_$STAMP}
mkdir -p "$OUT"
...
gpu_id=$GPU_ID
pairs=$PAIRS
candidate=group128-oct-row-u32x2
EOF
...
run_shape() {
  ...
  env HIP_VISIBLE_DEVICES="$GPU_ID" "$BIN" \
    ...
    --group128-oct-row-u32x2 "${extra[@]}" 2>&1 | tee "$log"

...
OUT_DIR="${OUT_DIR:-${ROOT}/experiments/gfx11-gate-up-x256y64/results/pp8192_residual_incremental_ab}"
...
run_one() {
    ...
    timeout --signal=INT --kill-after=5s 180s \
        ...
        HIPFIRE_RDNA3_HFQ4_RESIDUAL_X256Y64="${residual}" \
        "${BIN}" "${MODEL}" \
...
for ((pair=1; pair<=TRIALS; pair++)); do
    if (( pair % 2 == 1 )); then
        modes=(gate gate_residual)
    else
        modes=(gate_residual gate)
    fi
    for order in 0 1; do

...
OUT_DIR="${OUT_DIR:-${ROOT}/experiments/gfx11-gate-up-x256y64/results/pp8192_group128_k128_ck_ab}"
...
run_command() {
    ...
    timeout --signal=INT --kill-after=5s 180s \
        ...
        HIPFIRE_RDNA3_Q8_GROUP128_K128="${k128}" \
...
run_one() {
    local pair="$1" order="$2" mode="$3" k128 log summary prefill gen token_ids
    [[ "${mode}" == "k32" ]] && k128=0 || k128=1
    log="${OUT_DIR}/pair_${pair}_${order}_${mode}.log"
    run_command "${log}" "${k128}"
...
echo "Prewarming K32 and K128 paths on HIP device ${GPU_ID}"
run_command "${OUT_DIR}/prewarm_k32.log" 0
sleep "${SLEEP_SECS}"
run_command "${OUT_DIR}/prewarm_k128.log" 1
sleep "${SLEEP_SECS}"
...
k32 = statistics.median(samples("k32", "prefill_tok_s", trim_values=True))
k128 = statistics.median(samples("k128", "prefill_tok_s", trim_values=True))
token_sets = {
    mode: {r["token_ids"] for r in rows if r["mode"] == mode}
    for mode in ("k32", "k128")
}
print(f"trim_each_side={trim} k32_trimmed_median={k32:.3f} k128_trimmed_median={k128:.3f}")
print(f"k128_vs_k32={k128 / k32:.4f}x ({(k128 / k32 - 1) * 100:+.2f}%)")
print(f"token_ids_match={token_sets['k32'] == token_sets['k128']} "
      f"k32_variants={len(token_sets['k32'])} k128_variants={len(token_sets['k128'])}")
PY

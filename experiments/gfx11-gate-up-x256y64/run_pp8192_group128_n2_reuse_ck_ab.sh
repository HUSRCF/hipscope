...
OUT_DIR="${OUT_DIR:-${ROOT}/experiments/gfx11-gate-up-x256y64/results/pp8192_group128_n2_reuse_ck_gpu1_${STAMP}}"
...
run_command() {
    ...
    timeout --signal=INT --kill-after=5s 240s env \
        ...
        HIPFIRE_RDNA3_Q8_GROUP128_N2_REUSE="${n2}" \
...
run_one() {
    local pair="$1" order="$2" mode="$3" n2=0
    [[ "${mode}" == n2 ]] && n2=1
...
sleep "${SLEEP_SECS}"
run_command "${OUT_DIR}/prewarm_n2.log" 1
sleep "${SLEEP_SECS}"

for ((pair=1; pair<=TRIALS; pair++)); do
    if ((pair % 2)); then modes=(baseline n2); else modes=(n2 baseline); fi
    for order in 0 1; do
...
n2 = statistics.median(values("n2", "prefill_tok_s", True))
tokens = {mode: {row["token_ids"] for row in rows if row["mode"] == mode}
          for mode in ("baseline", "n2")}
print(f"trim_each_side={trim} baseline_trimmed_median={baseline:.3f} n2_trimmed_median={n2:.3f}")
print(f"n2_vs_baseline={n2 / baseline:.4f}x ({(n2 / baseline - 1) * 100:+.2f}%)")
print(f"token_ids_match={tokens['baseline'] == tokens['n2']} "
      f"baseline_variants={len(tokens['baseline'])} n2_variants={len(tokens['n2'])}")
PY

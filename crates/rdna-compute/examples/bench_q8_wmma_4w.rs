...
fn main() {
    ...
    let default_shapes: &[(usize, usize, usize, &str)] = &[
    ...
    let qwen36_shapes: &[(usize, usize, usize, &str)] =
        &[(17408, 5120, 2048, "Qwen3.6 gate/up M=17408 K=5120 B=2048")];
    let shapes = if std::env::var("HIPFIRE_Q8_BENCH_QWEN36").as_deref() == Ok("1") {
        qwen36_shapes
    } else {
        default_shapes
    };
    ...
    for &(m, k, n, label) in shapes {
        ...
        if k % 128 == 0 {
            ...
            gpu.hip
                .memcpy_htod(&xf32_gpu, &xf32_bytes)
                .expect("htod Xf32");
            ...
            let yi8: &[f32] =
                unsafe { std::slice::from_raw_parts(yb.as_ptr() as *const f32, n * m) };

...
//! Focused gfx11 X256/Y64 group128 8-wave versus 16-wave probe.
...
fn main() {
    ...
    let y8 = gpu.upload_f32(&zeros, &[n, m]).expect("8-wave output");
    let y16 = gpu.upload_f32(&zeros, &[n, m]).expect("16-wave output");
    ...
    let run8 = |gpu: &mut Gpu| {
        gpu.gemm_hfq4g256_mmq_set_prequant_x256y64_perm_group128(
            &a, xq, &y8, m, k, n,
        )
    };
    let run16 = |gpu: &mut Gpu| {
        gpu.gemm_hfq4g256_mmq_prequant_x256y64_perm_group128_wave16(
            &a, xq, &y16, m, k, n, false,
        )
    };

    run8(&mut gpu).expect("8-wave correctness");
    run16(&mut gpu).expect("16-wave correctness");
    gpu.hip.device_synchronize().expect("correctness sync");
    let max_abs = if skip_correctness {
        f32::NAN
    } else {
        let ref_host = gpu.download_f32(&y8).expect("download 8-wave");
        let candidate_host = gpu.download_f32(&y16).expect("download 16-wave");
        ref_host.iter().zip(candidate_host.iter())
            .map(|(a, b)| (a - b).abs()).fold(0.0f32, f32::max)
    };

    for _ in 0..3 {
        run8(&mut gpu).expect("8-wave warmup");
        run16(&mut gpu).expect("16-wave warmup");
    }
    gpu.hip.device_synchronize().expect("warmup sync");
    let mut ms8 = Vec::with_capacity(pairs);
    let mut ms16 = Vec::with_capacity(pairs);
    for pair in 0..pairs {
        for candidate_first in [pair % 2 != 0, pair % 2 == 0] {
            let start = Instant::now();
            if candidate_first {
                run16(&mut gpu).expect("timed 16-wave");
            } else {
                run8(&mut gpu).expect("timed 8-wave");
            }
            gpu.hip.device_synchronize().expect("timed sync");
            let ms = start.elapsed().as_secs_f64() * 1_000.0;
            if candidate_first { ms16.push(ms); } else { ms8.push(ms); }
        }
    }

    let med8 = median(ms8);
    let med16 = median(ms16);
    println!("m={m} k={k} n={n}");
    println!("wave8_ms={med8:.4}");
    println!("wave16_ms={med16:.4}");
    println!("wave16_speedup={:.4}x", med8 / med16);

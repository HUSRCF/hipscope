...
impl Gpu {
    ...
    /// Stage 3b: on-GPU DDTree build + linearize (single-thread kernel).
    ///
    /// Reads device-resident draft logits `[depth × vocab]`; writes:
    ///   top_tokens_out   [depth × topk] i32
    ///   top_log_probs_out[depth × topk] f32
    ///   attn_bias        [max_n × max_n] f32
    ///   parent_indices   [max_n]         i32 (4 bytes each, Raw tensor)
    ///   slot_depth       [max_n]         i32 (Raw tensor)
    ///   child_of_cand    [max_n × topk]  i32 (Raw tensor)
    ///   big_n_out        [1]             i32 (Raw tensor, 4-byte D2H readback)
    ///   node_tokens_out  [max_n]         i32 (Raw tensor)
    ///
    /// Grid: [1,1,1]. Block: [1,1,1]. Byte-identical to host tree build at temp=0.
    #[allow(clippy::too_many_arguments)]
    pub fn ddtree_build_and_linearize_f32(
        &mut self,
        logits: &GpuTensor,            // [depth × vocab] f32
        top_tokens_out: &GpuTensor,    // [depth × topk] i32
        top_log_probs_out: &GpuTensor, // [depth × topk] f32
        attn_bias: &GpuTensor,         // [max_n × max_n] f32
        parent_indices: &GpuTensor,    // [max_n] i32 (Raw, 4*max_n bytes)
        slot_depth: &GpuTensor,        // [max_n] i32 (Raw)
        child_of_cand: &GpuTensor,     // [max_n × topk] i32 (Raw)
        big_n_out: &GpuTensor,         // [1] i32 (Raw)
        node_tokens_out: &GpuTensor,   // [max_n] i32 (Raw)
        vocab: usize,
        depth: usize,
        topk: usize,
        max_nodes: usize,
        ...
        self.bind_thread()?;
        assert!(topk >= 1 && topk <= 8, "ddtree_build_and_linearize: topk={topk} out of [1,8]");
        assert!(depth >= 1 && depth <= 63, "ddtree_build_and_linearize: depth={depth} out of [1,63]");
        assert!(max_nodes <= 60, "ddtree_build_and_linearize: max_nodes={max_nodes} > 60");
        self.ensure_kernel(
            "ddtree_build_and_linearize",
            kernels::DDTREE_BUILD_AND_LINEARIZE_SRC,
            "ddtree_build_and_linearize_f32",
        )?;
        let func = &self.functions["ddtree_build_and_linearize_f32"];
        let mut lp  = logits.buf.as_ptr();
        let mut tto = top_tokens_out.buf.as_ptr();
        let mut tlp = top_log_probs_out.buf.as_ptr();
        let mut ab  = attn_bias.buf.as_ptr();
        let mut pi  = parent_indices.buf.as_ptr();
        let mut sd  = slot_depth.buf.as_ptr();
        let mut coc = child_of_cand.buf.as_ptr();
        let mut bno = big_n_out.buf.as_ptr();
        let mut nto = node_tokens_out.buf.as_ptr();
        let mut vv  = vocab as i32;
        let mut dd  = depth as i32;
        let mut kk  = topk as i32;
        let mut mn  = max_nodes as i32;
        let mut lc  = logw_cutoff;
        let mut params: Vec<*mut std::ffi::c_void> = vec![
            &mut lp  as *mut _ as *mut std::ffi::c_void,
            &mut tto as *mut _ as *mut std::ffi::c_void,
            &mut tlp as *mut _ as *mut std::ffi::c_void,
            &mut ab  as *mut _ as *mut std::ffi::c_void,
            &mut pi  as *mut _ as *mut std::ffi::c_void,
            &mut sd  as *mut _ as *mut std::ffi::c_void,
            &mut coc as *mut _ as *mut std::ffi::c_void,
            &mut bno as *mut _ as *mut std::ffi::c_void,
            &mut nto as *mut _ as *mut std::ffi::c_void,
            &mut vv  as *mut _ as *mut std::ffi::c_void,
            &mut dd  as *mut _ as *mut std::ffi::c_void,
            &mut kk  as *mut _ as *mut std::ffi::c_void,
            &mut mn  as *mut _ as *mut std::ffi::c_void,
            &mut lc  as *mut _ as *mut std::ffi::c_void,
        ];
        unsafe {
            self.hip.launch_kernel(
                func,

...
impl Gpu {
    ...
    pub fn attention_flash_asym3_batched_masked(
        ...
        if !self.graphs.capture_mode
            ...
            && n_kv_heads == 4
            && head_dim == 256
            && self.flash_attn_ck_quantized.is_some()
        {
            self.bind_thread()?;
            let sidecar = self.flash_attn_ck_quantized.as_ref().unwrap();
                    ...
                    && sidecar.is_staged_supported(&params).is_ok()
                {
                    unsafe { sidecar.staged_prefill(&params) }.map_err(|error| {
                        hip_bridge::HipError::new(
                            0,
                            &format!("staged quantized FlashAttention CK launch failed: {error}"),
                        )
                    })?;

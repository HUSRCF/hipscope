...
//!
//! Adapted from the old branch's `forward_scratch` / `sliding_layer_decode` /
//! `full_layer_decode` for the E2B/E4B dense text towers. Per-token pipeline:
//!
...
pub fn decode_step_capture(
    ...
    decode_step_body(
        ...
        gpu,
        position,
        Some(DecodeHiddenCapture::AllLayers(capture)),
    )
}

/// Decode one token while appending only selected post-layer residual rows in
/// layer order. This is the host-capture path used by target-generic DFlash;
/// limiting downloads to the draft's extract layers avoids the oracle dumper's
/// all-layer transfer cost.
pub fn decode_step_capture_selected(
    cfg: &Gemma4Config,
    weights: &Gemma4Weights,
    state: &mut Gemma4State,
    gpu: &mut Gpu,
    ...
    decode_step_body(
        ...
        gpu,
        position,
        Some(DecodeHiddenCapture::Selected { layers, hidden_out }),
    )?;
    gpu.download_f32(&state.logits)
        .map_err(|e| format!("gemma4 capture: download logits: {e:?}"))
}

/// Return the model-input embedding row used by Gemma4 after its sqrt(dim)
/// scaling. DFlash uses this for the committed seed and mask-token rows.
pub fn embedding_row(
    ...
    gpu.download_f32(&state.x)
        .map_err(|e| format!("gemma4 embedding row download: {e:?}"))
}

/// Apply Gemma4's final norm, tied lm_head, and logit softcap to `n` residual
/// hidden rows. This mirrors the eager decode tail and is the target-lm-head
/// seam consumed by the generic DFlash drafter.
pub fn lm_head_logits_rows(
    cfg: &Gemma4Config,
    weights: &Gemma4Weights,
    state: &mut Gemma4State,
    gpu: &mut Gpu,
    ...
    for row in 0..n {
        gpu.hip
            .memcpy_dtod_at(&state.x.buf, 0, &hidden_rows.buf, row * dim * 4, dim * 4)
            .map_err(|e| format!("gemma4 lm_head row {row} copy: {e:?}"))?;
        gpu.rmsnorm_f32(&state.x, &weights.final_norm, &state.tmp, cfg.norm_eps)
            .map_err(|e| format!("gemma4 lm_head row {row} norm: {e:?}"))?;
        weight_gemv(gpu, &weights.lm_head, &state.tmp, &state.logits)
            .map_err(|e| format!("gemma4 lm_head row {row}: {e}"))?;
        if cfg.final_logit_softcapping > 0.0 {
            gpu.logit_softcap_f32(&state.logits, vocab, cfg.final_logit_softcapping)
                .map_err(|e| format!("gemma4 lm_head row {row} softcap: {e:?}"))?;
        }
        out.extend_from_slice(
            &gpu.download_f32(&state.logits)
                .map_err(|e| format!("gemma4 lm_head row {row} download: {e:?}"))?,
        );
    }
    Ok(out)
}

/// Decode one token via hipGraph capture/replay. Default is conservative for
/// Gemma4 E-series PLE/KV-sharing configs; use `HIPFIRE_GEMMA4_GRAPH=1` to
/// force graph experiments and `HIPFIRE_GEMMA4_GRAPH=0` to force eager.
/// The layer body + final-norm + lm_head are captured once and replayed per
/// token, recovering per-token host launch overhead.
///
...
pub fn decode_step_with_graph(
    ...
    let env_override = match std::env::var("HIPFIRE_GEMMA4_GRAPH").ok().as_deref() {
        Some("1") => Some(true),
        Some("0") => Some(false),
        _ => None,
    };
    let default_graph_on = cfg.hidden_size_per_layer_input == 0 && cfg.num_kv_shared_layers == 0;
    let graph_on = env_override.unwrap_or(default_graph_on);
    if !graph_on {
    ...
    // Embedding lookup + PLE projection OUTSIDE the captured region — token_id
    // is baked into embedding kernargs. Runs on the active stream, ordered
    // before the captured body that reads `state.x` and PLE slices.
    prepare_token_inputs(cfg, weights, state, gpu, token_id)?;
...
fn prepare_per_layer_inputs(
    ...
    if (token_id as usize) >= cfg.vocab_size_per_layer_input {
        return Err(format!(
            "gemma4: PLE token id {} out of range for vocab_size_per_layer_input {}",
            token_id, cfg.vocab_size_per_layer_input
        ));
    }

    // HF get_per_layer_inputs(): embed_tokens_per_layer(input_ids), whose
    // embedding module scales by sqrt(hidden_size_per_layer_input).
    embedding_lookup_to(
        gpu,
        ple.embd_format,
        &ple.embed_tokens,
    ...
    // HF project_per_layer_inputs(): per_layer_model_projection(inputs_embeds)
    // * hidden_size^-0.5, reshape [layers, ple_dim], RMSNorm over ple_dim.
    ...
    // Combine token-identity and context projection: (a + b) * 1/sqrt(2).
...
fn prepare_per_layer_inputs_batched(
    ...
    for (i, &token_id) in tokens.iter().enumerate() {
        if (token_id as usize) >= cfg.vocab_size_per_layer_input {
            return Err(format!(
                "gemma4 forward_batch: PLE token id {} out of range for vocab_size_per_layer_input {}",
                token_id, cfg.vocab_size_per_layer_input
            ));
        }
        let token_row = token_inputs.sub_offset(i * packed_dim, packed_dim);
        embedding_lookup_to(
            gpu,
            ple.embd_format,
            &ple.embed_tokens,
        ...
        // `per_layer_model_projection` is BF16/F32 on Gemma4-2B and the
        // generic batched projection helper intentionally handles quantized
        // projection kernels only. Use the proven single-row path first; this
        // keeps batch verify correctness independent of a new F32 GEMM kernel.
        let x_row = x.sub_offset(i * cfg.dim, cfg.dim);
        let projection_row = projection_all.sub_offset(i * packed_dim, packed_dim);
        weight_gemv(gpu, &ple.model_projection, &x_row, &projection_row)
            .map_err(|e| format!("gemma4 forward_batch ple model_projection row {i}: {e}"))?;
    }
...
enum DecodeHiddenCapture<'a> {
    AllLayers(&'a mut [Vec<f32>]),
    Selected {
        layers: &'a [usize],
...
fn decode_step_body(
    ...
    gpu: &mut Gpu,
    position: u32,
    mut capture: Option<DecodeHiddenCapture<'_>>,
) -> Result<(), String> {
    ...
    for layer_idx in 0..cfg.n_layers {
        ...
        if let Some(cap) = capture.as_mut() {
            match cap {
                DecodeHiddenCapture::AllLayers(rows) => {
                    let h = gpu
                        .download_f32(&state.x)
                        .map_err(|e| format!("gemma4 L{layer_idx}: capture download: {e:?}"))?;
                    rows[layer_idx].extend_from_slice(&h);
                }
                DecodeHiddenCapture::Selected { layers, hidden_out }
                    if layers.binary_search(&layer_idx).is_ok() =>
                {
                    let h = gpu.download_f32(&state.x).map_err(|e| {
                        format!("gemma4 L{layer_idx}: selected capture download: {e:?}")
                    })?;
                    hidden_out.extend_from_slice(&h);
                }
                DecodeHiddenCapture::Selected { .. } => {}
            }
        }
    }
    state.n_tokens = position as usize + 1;
...
fn sliding_layer_decode(
    ...
    if shared_source_slot.is_some() {
        ...
        // n1 = input_layernorm(x) -> q/k/v. Fused
        // (norm+rotate+prerotated GEMVs, shared rotate) when MQ4G256; else
        // plain rmsnorm -> rotation-doing GEMVs.
        attn_input_qkv(
            gpu,
            &lw.input_layernorm,
            &state.tmp,
    ...
    // KV write (Q8) + windowed attention (window = sliding_window). Shared
    // layers only attend to the cached K/V from the last non-shared same-type
    // source layer.
    attn_q8_swa(
        gpu,
...
/// One full (global) attention layer (head_dim 512, partial RoPE).
fn full_layer_decode(
    ...
    if shared_source_slot.is_some() {
        ...
        // n1 = input_layernorm(x) -> q/k(/v). Fused
        // (norm+rotate+prerotated GEMVs, shared rotate) when MQ4G256; else
        // plain rmsnorm -> rotation-doing GEMVs. When v_proj is Some, v is
        // projected inside the helper from the same (pre)normed input; when
        // None (k_eq_v) the helper projects only q+k and we capture V below
        // from the PRE-k_norm k output.
        attn_input_qkv(
            gpu,
            &lw.input_layernorm,
            &state.tmp,
        ...
        // V handling for the k_eq_v (12B) case: V = K's PRE-k_norm output
        // (memcpy k -> v BEFORE applying k_norm). Then weight-less RMSNorm on
        // V (below).
        ...
        // Weight-less V RMSNorm (ones). For k_eq_v, V was already captured
        // from the PRE-k_norm K above, so this is safe to run before the fused
        // q/k path.
        gpu.rmsnorm_batched(&state.v, &state.v_norm_ones, &state.v, n_kv, head_dim, eps)
    ...
    // KV write (Q8) + full causal attention (window = 0). Shared layers only
    // attend to the cached K/V from the last non-shared same-type source layer.
    attn_q8_swa(
        gpu,
        &mut state.kv_full,
        if shared_source_slot.is_some() {
...
fn apply_per_layer_input_branch(
    ...
    // HF Gemma4TextDecoderLayer PLE branch:
    // residual = hidden_states
    // hidden = gelu(per_layer_input_gate(hidden_states)) * per_layer_input_i
    // hidden = per_layer_projection(hidden)
    // hidden = post_per_layer_input_norm(hidden)
...
///                 ~61 ms/round on gfx1201 batch-decode probes; elsewhere
///                 identical scalar kernel, b ≤ 64 = one sub-batch)
...
///
/// Thin wrapper over `forward_batch_impl` with optional out-params off:
...
pub fn forward_batch(
    ...
    forward_batch_impl(
        cfg, weights, state, gpu, tokens, start_pos, None, None, None,
    )
}

/// Batched verify forward returning the target greedy argmax at every block
/// position. This is the narrow SpecTarget-facing wrapper over the existing
/// diagnostic path; it keeps Gemma4 speculative-decode plumbing from depending
/// on `forward_batch_impl` internals.
...
/// Diagnostic variant of [`forward_batch_argmax`] that also returns the
/// scalar-GEMV logits already computed for the final row by
/// [`forward_batch_impl`]. This lets parity probes compare the batched Q8
/// lm-head decision with the eager-compatible final-row lm-head without a
/// second model forward.
pub fn forward_batch_argmax_with_last_logits(
    cfg: &Gemma4Config,
    weights: &Gemma4Weights,
    state: &mut Gemma4State,
    gpu: &mut Gpu,
    ...
    let mut argmax = Vec::new();
    let last_logits = forward_batch_impl(
        cfg,
        weights,
        state,
        gpu,
    ...
    Ok((argmax, last_logits))
}

/// Batched verifier returning target argmax at every position while writing
/// each position's final-RMSNorm hidden to `final_hidden_out` as `[B, dim]`.
/// The Gemma4 E2B assistant consumes row `accepted_drafts` after verification;
/// keeping the rows device-resident avoids a hidden-state D2H/H2D round trip.
pub fn forward_batch_argmax_final_hidden(
    cfg: &Gemma4Config,
    weights: &Gemma4Weights,
    state: &mut Gemma4State,
    gpu: &mut Gpu,
    tokens: &[u32],
    start_pos: usize,
    final_hidden_out: &GpuTensor,
) -> Result<Vec<u32>, String> {
    let needed = tokens.len() * cfg.dim;
    if final_hidden_out.dtype != DType::F32 || final_hidden_out.numel() < needed {
        return Err(format!(
            "gemma4 forward_batch final hidden output has dtype={:?}, numel={}, expected F32 with at least {needed}",
            final_hidden_out.dtype,
            final_hidden_out.numel()
        ));
    }
    let mut argmax = Vec::new();
    let _ = forward_batch_impl(
...
/// Batched verifier with selected post-layer residual capture. `hidden_out` is
/// appended position-major as `[B, layers.len(), dim]`, matching the generic
/// DFlash target-hidden cache contract.
#[allow(clippy::too_many_arguments)]
pub fn forward_batch_argmax_capture(
    cfg: &Gemma4Config,
    weights: &Gemma4Weights,
    state: &mut Gemma4State,
    gpu: &mut Gpu,
    ...
    let _ = forward_batch_impl(
        ...
        start_pos,
        None,
        Some((layers, hidden_out)),
        Some(&mut argmax),
    )?;
    Ok(argmax)
}

/// Batched verify forward with optional diagnostic out-params. Identical body
/// to `forward_batch`; the two optional outputs are computed ADDITIVELY after
/// the layer loop and do not alter the returned last-token logits, the KV
/// writes, or `state.n_tokens`.
///
...
///   `final_norm` (the lm_head input).
/// * `selected_capture` — receives selected post-layer residual rows in
///   position-major `[B, selected_layers, dim]` order for generic DFlash.
...
///   logit softcap the eager path uses, per row.
///
...
fn forward_batch_impl(
    ...
    selected_capture: Option<(&[usize], &mut Vec<f32>)>,
    ...
    let seq_end = start_pos
        .checked_add(b)
        .ok_or_else(|| "gemma4 forward_batch: sequence position overflow".to_string())?;
    if seq_end > state.max_seq {
        return Err(format!(
            "gemma4 forward_batch: sequence end {seq_end} exceeds allocated KV capacity {}",
            state.max_seq
        ));
    }
    let dim = cfg.dim;
    ...
    // The shared Q8 tree-mask kernel derives its effective block start from
    // positions[0] (rather than the scalar block_start) for graph replay.  This
    // verifier supplies a full-history [B, start_pos+B] mask beginning at cache
    // slot zero, so attention needs a separate origin buffer.  RoPE and KV
    // writes continue to use the absolute positions above.
    let mask_origin_pos = alloc(gpu, b, "mask_origin_pos")?;
    let zero_positions = vec![0u8; b * std::mem::size_of::<i32>()];
    gpu.hip
        .memcpy_htod(&mask_origin_pos.buf, &zero_positions)
        .map_err(|e| format!("gemma4 forward_batch htod mask origin: {e:?}"))?;
    ...
    let capture_layers = selected_capture.as_ref().map(|(layers, _)| *layers);
    let mut captured_layer_rows: Vec<Vec<f32>> = Vec::new();
    for layer_idx in 0..cfg.n_layers {
        let slot = state.kv_slot_for_layer[layer_idx];
        let shared_source_slot = match cfg.kv_shared_source_layer_idx(layer_idx) {
            Some(source_layer) => Some(state.kv_slot_for_layer[source_layer]),
            ...
            (LayerType::Sliding, LayerWeights::Sliding(lw)) => {
                ...
                batch_attn_block(
                    ...
                    BatchAttn {
                        ...
                        hd,
                        n_kv,
                        write_slot: if shared_source_slot.is_some() {
                            None
                        } else {
                            Some(slot)
                        },
        ...
        if capture_layers.is_some_and(|layers| layers.binary_search(&layer_idx).is_ok()) {
            captured_layer_rows.push(gpu.download_f32(&x).map_err(|e| {
                format!("gemma4 forward_batch L{layer_idx} selected capture: {e:?}")
            })?);
        }
    }
    if let Some((layers, hidden_out)) = selected_capture {
        if captured_layer_rows.len() != layers.len() {
            return Err(format!(
                "gemma4 forward_batch captured {} layers, expected {} ({layers:?})",
                captured_layer_rows.len(),
                layers.len()
            ));
        }
        hidden_out.reserve(b * layers.len() * dim);
        for row in 0..b {
            for layer_rows in &captured_layer_rows {
                hidden_out.extend_from_slice(&layer_rows[row * dim..(row + 1) * dim]);
            }
        }
    }
    state.n_tokens = start_pos + b;

    // ── Optional diagnostic out-params (ADDITIVE; both default-off). ──
    ...
    if want_hidden || want_argmax {
        ...
        if let Some(out) = per_pos_argmax_out {
            ...
            if weights.lm_head.gpu_dtype == DType::Q8_0 {
                ...
                .map_err(|e| format!("gemma4 forward_batch batched lm_head: {e:?}"))?;
                ...
                gpu.argmax_f32_batched(&logits_b, &idx_buf, vocab, b)
                    .map_err(|e| format!("gemma4 forward_batch batched argmax: {e:?}"))?;
                ...
                gpu.hip
                    ...
                    .map_err(|e| format!("gemma4 forward_batch argmax dtoh: {e:?}"))?;
                for v in idx_i32 {
                    out.push(v as u32);
                }
                ...
                for i in 0..b {
                    ...
                    weight_gemv(gpu, &weights.lm_head, &hidden_row, &state.logits)
                        .map_err(|e| format!("gemma4 forward_batch lm_head row {i}: {e}"))?;
                    if cfg.final_logit_softcapping > 0.0 {
                        gpu.logit_softcap_f32(&state.logits, vocab, cfg.final_logit_softcapping)
                            .map_err(|e| format!("gemma4 forward_batch softcap row {i}: {e:?}"))?;
                    }
                    let row = gpu
                        .download_f32(&state.logits)
                        .map_err(|e| format!("gemma4 forward_batch download row {i}: {e:?}"))?;
...
fn batch_attn_block(
    ...
    // q / k projections (shared nrm input). KV-sharing layers only need Q;
    // K/V are read from the last non-shared source slot and no cache write is
...
fn apply_per_layer_input_branch_batched(
    ...
    // The PLE branch weights are BF16/F32 on the Gemma4-2B smoke model. Use
    // row-wise proven GEMV for correctness; production tuning can replace this
    // with a dedicated small-M batched F32 projection later.
    for row in 0..b {

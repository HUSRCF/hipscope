    ...
    pub(super) fn max_compress_pos() -> usize {
        ...
        *V.get_or_init(|| {
            hipfire_config::developer_var("HIPFIRE_DEEPSEEK4_MAX_COMPRESS_POS")
                ...
                .map(|v: usize| v.clamp(1, i32::MAX as usize))
                .unwrap_or(2048)
        })
    }
    ...
    match weight.dtype {
        ...
        DType::F16 => {
            ...
            if gpu.arch_caps.has_wmma_w32() {
                let scratch = x_f16_scratch
                    .ok_or_else(|| "F16 weight requires x_f16_scratch on gfx11".to_string())?;
                ...
                gpu.gemm_f16_batched_lmhead(weight, x_plain_batch, y, m, k, batch_size)
                    .map_err(|e| format!("gemm_f16_batched_lmhead: {e:?}"))
            }
        }
        _ => {
            let wmma_on = hipfire_config::developer_var("HIPFIRE_DEEPSEEK4_HFQ4_WMMA")
                .map(|s| s != "0")
                .unwrap_or(true);
            if wmma_on && gpu.arch_caps.has_wmma_w32() {
...
fn compressor_forward_impl(
    ...
    // Decode/graph path: select the APE row from a device position buffer so
    // graph replay does not retain the row chosen during capture. Batched
    // prefill still selects its explicit host-known row above.
    let ape_pos_buf = if pre_batched.is_none() {
        Some(pos_slot(state, layer_idx, 0)?)
    } else {
        None
    };
    let mut ape_fused = false;
    let owned_kv_buf;
    let owned_score_buf;
    let projection_probe = ds4_fine_probe_start(gpu, layer_idx, state.n_tokens)?;
    let (kv_buf, score_buf) = if let Some((kv_b, score_b, b_off)) = pre_batched {
        ...
        static GFX90A_COMP_TWIN_Q8: std::sync::OnceLock<bool> = std::sync::OnceLock::new();
        static GFX90A_COMP_TWIN_F16_WAVE64_APE: std::sync::OnceLock<bool> =
            std::sync::OnceLock::new();
        static GFX90A_COMP_TWIN_Q8_WAVE64_APE: std::sync::OnceLock<bool> =
            std::sync::OnceLock::new();
        let twin_q8 = gpu.arch == "gfx90a"
            && wkv.dtype == DType::Q8_0
            && wgate.dtype == DType::Q8_0
            && *GFX90A_COMP_TWIN_Q8.get_or_init(|| {
                hipfire_config::developer_var("HIPFIRE_GFX90A_DS4_COMP_TWIN_Q8").as_deref()
                    == Ok("1")
            });
        let q8_wave64_ape = twin_q8
            && *GFX90A_COMP_TWIN_Q8_WAVE64_APE.get_or_init(|| {
                hipfire_config::developer_var("HIPFIRE_GFX90A_DS4_COMP_TWIN_Q8_WAVE64_APE")
                    .as_deref()
                    == Ok("1")
            });
        let f16_wave64_ape = gpu.arch == "gfx90a"
            && wkv.dtype == DType::F16
            && wgate.dtype == DType::F16
        ...
        if f16_wave64_ape {
            gpu.fused_twin_f16_xf32_wave64_bias_gfx90a(
                ...
                scb,
                ape,
                ape_pos_buf.as_ref().unwrap(),
                ratio as i32,
                proj_dim,
                proj_dim,
                hidden,
            )
            .map_err(|e| format!("gfx90a compressor wave64 twin F16 l{layer_idx}: {e:?}"))?;
            ape_fused = true;
        } else if q8_wave64_ape {
            ...
            .map_err(|e| format!("gfx90a compressor wave64 twin Q8 l{layer_idx}: {e:?}"))?;
            ape_fused = true;
        } else if twin_q8 {
            gpu.fused_gate_up_q8_0(wkv, wgate, tmp_plain, kvb, scb, proj_dim, proj_dim, hidden)
                .map_err(|e| format!("gfx90a compressor twin Q8 l{layer_idx}: {e:?}"))?;
        } else {
    ...
    ds4_fine_probe_end(
        ...
        layer_idx,
        if is_indexer {
            "compressor_index_projection"
        } else {
            "compressor_main_projection"
        },
    )?;
    let state_probe = ds4_fine_probe_start(gpu, layer_idx, state.n_tokens)?;
    ...
    if !ape_fused {
        ...
        if let Some(pos_buf) = ape_pos_buf.as_ref() {
            gpu.compressor_add_ape_pos_buf_f32(
                &score_view,
                ape,
                pos_buf,
                ratio as i32,
            ...
            .map_err(|e| format!("comp graph-safe ape add l{layer_idx}: {e:?}"))?;
        } else {
    ...
    // Upstream shares the compressed layer's YaRN `freqs_cis` between the
    // main attention, Indexer Q, and Indexer compressor K.  In particular,
    // Indexer K is not plain theta=compress_rope_theta RoPE.
    ...
    if paired_ring {
        ...
        .map_err(|e| format!("comp paired ring write l{layer_idx}: {e:?}"))?;
    } else {
    ...
    if overlap {
        ...
        if paired_concat_shift {
            ...
            .map_err(|e| format!("comp paired concat l{layer_idx}: {e:?}"))?;
        } else {
            gpu.compressor_overlap_concat_f32(kv_state, concat_kv, ratio as i32, head_dim as i32)
                .map_err(|e| format!("comp concat_kv l{layer_idx}: {e:?}"))?;
            gpu.compressor_overlap_concat_f32(
    ...
    // Diagnostics must not construct a view past the fixed compressed-cache
    // capacity once the cache has stopped accepting commits.
    let commit_row = if comp_dump_here && pos / ratio < max_compressed {
        Some(kv_cache.sub_offset((pos / ratio) * head_dim, head_dim))
    } else {
        None
    };
    if let Some(row) = commit_row.as_ref() {
        comp_dbg(&*gpu, "kv_cache(pool)", row, head_dim);
    }
    ...
    if let Some(row) = commit_row.as_ref() {
        comp_dbg(&*gpu, "kv_cache(rmsnorm)", row, head_dim);
    }
    if do_rope {
    ...
    if let Some(row) = commit_row.as_ref() {
        comp_dbg(&*gpu, "kv_cache(rope)", row, head_dim);
    }
    if overlap {
        if paired_concat_shift {
            ...
            .map_err(|e| format!("comp paired state shift buf l{layer_idx}: {e:?}"))?;
        } else {
...
fn compressor_forward_batched(
    ...
    if aligned && n_events_capped > 0 {
        ...
        .map_err(|e| format!("comp rope batched l{layer_idx}: {e:?}"))?;
...
fn indexer_forward(
    ...
    // 2. Tail YaRN on q_idx.  Upstream assigns the Indexer the exact same
    //    compressed-layer `freqs_cis` as the main attention and Indexer K.
    //    Use main `pos_buf` (already holds the current position from
    //    apply_tail_rope); qk_rope_head_dim applies on each head.
    let pos_buf = state
        .pos_buf
        .as_ref()
    ...
    .map_err(|e| format!("idx yarn rope l{layer_idx}: {e:?}"))?;
    ...
    ds4_fine_probe_end(gpu, projection_probe, layer_idx, "indexer_projection")?;
    let selection_probe = ds4_fine_probe_start(gpu, layer_idx, state.n_tokens)?;
...
pub fn decode_step_with_graph(
    ...
    // 200+ steps on gfx1151 and 32-step EP decode on gfx90a. Default ON for
    // gfx90a and RDNA3+; opt out with `HIPFIRE_DEEPSEEK4_GRAPH=0`. Other
    // older architectures remain opt-in with `HIPFIRE_DEEPSEEK4_GRAPH=1`.
    ...
    let graph_on = env_override.unwrap_or_else(|| {
        ...
        a == "gfx90a" || a.starts_with("gfx11") || a.starts_with("gfx12")
    });
...
/// (graph replay path).
fn attn_state_values(win: i32, topk: i32, max_compressed: i32, pos: i32) -> [i32; 10] {
    let swa_slot = pos % win;
    let n_valid_swa = (pos + 1).min(win);
    let n_compressed_4 = ((pos + 1) / 4).min(max_compressed);
    let n_compressed_128 = ((pos + 1) / 128).min(max_compressed);
...
fn fill_attn_state_host(cfg: &DeepseekV4Config, state: &mut DeepseekV4State, position: u32) {
    let values = attn_state_values(
        cfg.sliding_window as i32,
        cfg.index_topk as i32,
        config_cache::max_compress_pos() as i32,
        position as i32,
    );
    let host = state
        .attn_state_host
    ...
    for layer_idx in 0..cfg.num_hidden_layers {
        ...
        if !skip_ffn {
            ...
            if layer_idx < cfg.num_hash_layers {
                ...
                ffn_routed(cfg, weights, state, gpu, layer_idx, None, false)?;
            }
        } else {
...
fn ds4_probe_layer_selected(layer: usize) -> bool {
    static LAYERS: std::sync::OnceLock<Option<Vec<usize>>> = std::sync::OnceLock::new();
    LAYERS
        .get_or_init(|| {
            let raw = hipfire_config::developer_var("HIPFIRE_DS4_PROBE_LAYERS").ok()?;
...
fn ds4_probe_warmup_steps() -> u64 {
    static STEPS: std::sync::OnceLock<u64> = std::sync::OnceLock::new();
    *STEPS.get_or_init(|| {
        hipfire_config::developer_var("HIPFIRE_DS4_PROBE_WARMUP_STEPS")
            .ok()
...
fn ds4_probe_step_selected(step: u64) -> bool {
    use std::sync::atomic::{AtomicU64, Ordering};
    static FIRST_STEP: AtomicU64 = AtomicU64::new(u64::MAX);
    let previous = FIRST_STEP.fetch_min(step, Ordering::Relaxed);
    let first = previous.min(step);
    step.saturating_sub(first) >= ds4_probe_warmup_steps()
}

type Ds4ProbeStart = Option<(std::time::Instant, u64)>;

fn ds4_coarse_probe_start(gpu: &mut Gpu, layer: usize, step: u64) -> Result<Ds4ProbeStart, String> {
    static ENABLED: std::sync::OnceLock<bool> = std::sync::OnceLock::new();
    let enabled = *ENABLED.get_or_init(|| {
        hipfire_config::developer_var("HIPFIRE_DS4_COARSE_PROBE").as_deref() == Ok("1")
    });
    if !enabled || !ds4_probe_layer_selected(layer) || !ds4_probe_step_selected(step) {
        return Ok(None);
    }
    gpu.hip
        .device_synchronize()
        .map_err(|e| format!("DS4 coarse probe pre-sync: {e:?}"))?;
    Ok(Some((std::time::Instant::now(), step)))
}

fn ds4_coarse_probe_end(
    gpu: &mut Gpu,
    started: Ds4ProbeStart,
    layer: usize,
    stage: &str,
) -> Result<(), String> {
    ...
    gpu.hip
        .device_synchronize()
        .map_err(|e| format!("DS4 coarse probe post-sync: {e:?}"))?;
    eprintln!(
        "DS4-COARSE step={step} layer={layer} rank={} stage={stage} ms={:.3}",
        gpu.device_id,
...
fn ds4_fine_probe_enabled() -> bool {
    static ENABLED: std::sync::OnceLock<bool> = std::sync::OnceLock::new();
    *ENABLED.get_or_init(|| {
        hipfire_config::developer_var("HIPFIRE_DS4_FINE_PROBE").as_deref() == Ok("1")
    })
}

fn ds4_fine_probe_start(gpu: &mut Gpu, layer: usize, step: u64) -> Result<Ds4ProbeStart, String> {
    if !ds4_fine_probe_enabled()
        || !ds4_probe_layer_selected(layer)
        || !ds4_probe_step_selected(step)
    {
        return Ok(None);
    }
    gpu.hip
        .device_synchronize()
        .map_err(|e| format!("DS4 fine probe pre-sync: {e:?}"))?;
    Ok(Some((std::time::Instant::now(), step)))
}

fn ds4_fine_probe_end(
    gpu: &mut Gpu,
    started: Ds4ProbeStart,
    layer: usize,
    stage: &str,
    ...
    gpu.hip
        .device_synchronize()
        .map_err(|e| format!("DS4 fine probe post-sync: {e:?}"))?;
    eprintln!(
        "DS4-FINE step={step} layer={layer} rank={} stage={stage} ms={:.3}",
        gpu.device_id,
        started.elapsed().as_secs_f64() * 1000.0,
    );
    Ok(())
...
fn ds4_moe_block_core(
    ...
    if !skip_ffn {
        ...
        if layer_idx < cfg.num_hash_layers {
            ...
            ffn_routed(cfg, weights, state, gpu, layer_idx, routed_out, false)?;
        }
        ds4_coarse_probe_end(gpu, probe, layer_idx, "routed_moe")?;
    } else {
        if state.ffn_out.is_none() {
            state.ffn_out = Some(
                gpu.alloc_tensor(&[cfg.hidden_size], DType::F32)
    ...
    if do_mix {
        let probe = ds4_coarse_probe_start(gpu, layer_idx, state.n_tokens)?;
        hc_ffn_mix(cfg, weights, state, gpu, layer_idx)?;
        ds4_coarse_probe_end(gpu, probe, layer_idx, "ffn_mix")?;
    }
    Ok(())
}
...
fn ep_trace_boundary(
    ...
    let enabled = hipfire_config::developer_var("HIPFIRE_EP_DUMP_BOUNDARIES_POS")
        .ok()
        .map(|s| {
            s.split(',')
    ...
    gpu.hip
        .device_synchronize()
        .map_err(|e| DispatchError::Hip(format!("EPBOUND sync: {e:?}")))?;
    let values = gpu
        .download_f32(tensor)
        .map_err(|e| DispatchError::Hip(format!("EPBOUND download: {e:?}")))?;
    let l2 = values
        .iter()
        .map(|&x| (x as f64) * (x as f64))
        .sum::<f64>()
    ...
    eprintln!(
        "EPBOUND pos={position} layer={layer} rank={} stage={stage} l2={l2:.9e} max={max_abs:.9e} fnv=0x{hash:016x}",
        gpu.device_id
    );
    Ok(())
}
...
fn ep_trace_moe_f32(
    ...
    let enabled = hipfire_config::developer_var("HIPFIRE_EP_DUMP_MOE_POS")
        .ok()
        .map(|value| {
            value
                .split(',')
                .any(|item| item.trim().parse::<u64>() == Ok(position))
        })
        .unwrap_or(false);
    if !enabled {
        return Ok(());
    ...
    gpu.hip
        .device_synchronize()
        .map_err(|error| format!("EPMOE sync: {error:?}"))?;
    let values = gpu
        .download_f32(tensor)
        .map_err(|error| format!("EPMOE download {stage}: {error:?}"))?;
    let l2 = values
        .iter()
    ...
    let prefix: Vec<String> = values
        .iter()
        .take(8)
        .map(|value| format!("{value:.6e}"))
        .collect();
    eprintln!(
        "EPMOE pos={position} layer={layer} rank={} stage={stage} n={} l2={l2:.9e} max_abs={max_abs:.9e} range=[{min:.9e},{max:.9e}] first=[{}]",
        gpu.device_id,
        values.len(),
        prefix.join(","),
    );
...
fn ep_trace_moe_i32(
    ...
    let values: Vec<i32> = bytes
        ...
        .collect();
    eprintln!(
        "EPMOE pos={position} layer={layer} rank={} stage={stage} values={values:?}",
        gpu.device_id,
    );
    Ok(())
}
...
impl<'a> ForwardBindings for Deepseek4Bindings<'a> {
    ...
    fn run_attend(
        ...
        .map_err(DispatchError::Hip)?;
        ds4_coarse_probe_end(gpu, probe, self.layer_idx, "attention").map_err(DispatchError::Hip)
    }
    fn run_moe(
        &mut self,
        gpu: &mut Gpu,
    ...
    fn run_moe_ep(
        ...
        ep_trace_boundary(
            ...
            self.position,
            self.layer_idx,
            "local_routed_partial",
        )
    }
    fn supports_moe_ep_rows(&self) -> bool {
        if self.skip_ffn || self.layer_idx < self.cfg.num_hash_layers {
            return false;
        }
        let layer = self.weights.resolve_layer(self.layer_idx);
        layer.expert_gate_up_dtype == Some(DType::MQ2G256Lloyd)
            && layer.expert_down_dtype == Some(DType::MQ2G256Lloyd)
    }
...
pub fn forward_ep(
    ...
    let dump_pos_hit = hipfire_config::developer_var("HIPFIRE_EP_DUMP_POS")
        ...
        .unwrap_or(false);
    // Optional narrow MoE probe layered on top of HIPFIRE_EP_DUMP_POS.  This
    // keeps the large raw tensor dumps out of ordinary boundary traces while
    // allowing one suspect routed layer to be recomputed independently on the
    // CPU from the exact activation and official gate tensor.
    let dump_moe_layer = hipfire_config::developer_var("HIPFIRE_EP_DUMP_MOE_LAYER")
        .ok()
            ...
            for r in 0..n {
                ...
                if let Some(t) = state_per_rank[r].residual_streams.as_ref() {
                    ...
                    if r == 0 {
                        dump_buf(
                            &mut gpus.devices[r],
                            &format!("ep_scalar_pos{position}_l{l}_end_streams"),
                            t,
                        );
                    }
                }
                if dump_moe_layer == Some(l) {
                    let prefix = format!("ep_scalar_pos{position}_l{l}_rank{r}");
                    if let Some(tensor) = state_per_rank[r].ffn_x_plain.as_ref() {
                        dump_buf(
                            &mut gpus.devices[r],
                            &format!("{prefix}_ffn_x_plain"),
                            tensor,
                        );
                    }
                    if let Some(tensor) = state_per_rank[r].ffn_x_rot.as_ref() {
                        dump_buf(&mut gpus.devices[r], &format!("{prefix}_ffn_x_rot"), tensor);
                    }
                    if let Some(tensor) = state_per_rank[r].router_scores.as_ref() {
                        dump_buf(
                            &mut gpus.devices[r],
                            &format!("{prefix}_router_scores"),
                            tensor,
                        );
                    }
                    if let Some(tensor) = state_per_rank[r].moe_topk_indices.as_ref() {
                        dump_buf(
                            &mut gpus.devices[r],
                            &format!("{prefix}_topk_indices_i32"),
                            tensor,
                        );
                    }
                    if let Some(tensor) = state_per_rank[r].moe_topk_weights.as_ref() {
                        dump_buf(
                            &mut gpus.devices[r],
                            &format!("{prefix}_topk_weights"),
                            tensor,
                        );
                    }
                    if let Some(tensor) = state_per_rank[r].ffn_out.as_ref() {
                        dump_buf(&mut gpus.devices[r], &format!("{prefix}_ffn_out"), tensor);
                    }
                    dump_buf(
                        &mut gpus.devices[r],
                        &format!("{prefix}_routed_partial"),
                        &partials[r],
                    );
                    if r == 0 {
                        if let Some(tensor) =
                            weights_per_rank[r].resolve_layer(l).gate_bias.as_ref()
                        {
                            dump_buf(&mut gpus.devices[r], &format!("{prefix}_gate_bias"), tensor);
                        }
                    }
                }
    ...
    // rank-0 implementation.
    {
        ...
        let probe = ds4_coarse_probe_start(
            &mut gpus.devices[0],
        ...
        if dump_pos_hit {
            if let Some(final_norm) = state_per_rank[0].final_norm.as_ref() {
                dump_buf(
                    &mut gpus.devices[0],
                    &format!("ep_scalar_pos{position}_final_norm"),
                    final_norm,
                );
            }
            if let Some(logits) = state_per_rank[0].logits.as_ref() {
                dump_buf(
                    &mut gpus.devices[0],
                    &format!("ep_scalar_pos{position}_logits"),
                    logits,
                );
            }
        }
        ds4_coarse_probe_end(
            &mut gpus.devices[0],
            probe,
            cfg.num_hidden_layers,
            "final_norm_head",
...
/// Missing tid2eid is rejected: silently substituting shared-only changes the
/// model rather than providing a valid compatibility fallback.
///
...
fn ffn_routed(
    ...
    let gate_up_dtype = layer.expert_gate_up_dtype.ok_or_else(|| {
        format!("ffn_routed l{layer_idx}: routed gate/up format metadata is missing")
    })?;
    let down_dtype = layer.expert_down_dtype.ok_or_else(|| {
        format!("ffn_routed l{layer_idx}: routed down format metadata is missing")
    })?;
    ...
    let im = cfg.moe_intermediate_size;
    let ffn_x_rot = state.ffn_x_rot.as_ref().unwrap();
    let ffn_x_expert = if gate_up_dtype == DType::HFP4G32 {
        state.ffn_x_plain.as_ref().unwrap()
    } else {
        ffn_x_rot
    };
    let ffn_out = state.ffn_out.as_ref().unwrap();
    // Route-scale: cache only the optional environment override. The model
    // default is per-config and must not be frozen by whichever model runs
    // through this process first.
    use std::sync::OnceLock;
    static ROUTE_SCALE_OVERRIDE: OnceLock<Option<f32>> = OnceLock::new();
    let route_scale_override = *ROUTE_SCALE_OVERRIDE.get_or_init(|| {
        hipfire_config::developer_var("HIPFIRE_DEEPSEEK4_ROUTE_SCALE")
            .ok()
            .and_then(|s| s.parse().ok())
    });
    let route_scale = route_scale_override.unwrap_or(cfg.routed_scaling_factor);
    ...
    if layer.expert_gate_up_blob.is_some() {
        ...
        let i8dot = gate_up_dtype == DType::MQ2G256Lloyd
            && rdna_compute::mq2_i8dot::layer_enabled(Some(layer_idx));
        if i8dot && state.moe_x_a8.is_none() {
            let bytes = (cfg.hidden_size / 128) * 136;
            state.moe_x_a8 = Some(
                gpu.alloc_tensor(&[bytes], DType::Raw)
                    .map_err(|e| format!("alloc moe_x_a8: {e:?}"))?,
            );
        }
        ...
        let moe_params = hipfire_dispatch::families::moe::MoeBiasAwareParams {
            ...
            down_dtype,
            x_rot: ffn_x_expert,
            x_a8: if i8dot { state.moe_x_a8.as_ref() } else { None },
            ffn_out: out_target,
            scores: scores_dev,
            gate_bias: bias_dev,
        ...
        if prepare_only {
            hipfire_dispatch::pipeline::run_moe_decode_bias_aware_prepare(gpu, &moe_params)
        } else {
            hipfire_runtime::llama::moe_family().run_bias_aware(gpu, &moe_params)
        }
...
fn ffn_routed_rows(
    ...
    let layer = weights.resolve_layer(layer_idx);
    let gate_up_dtype = layer.expert_gate_up_dtype.ok_or_else(|| {
        format!("ffn_routed_rows l{layer_idx}: routed gate/up format metadata is missing")
    })?;
    let down_dtype = layer.expert_down_dtype.ok_or_else(|| {
        format!("ffn_routed_rows l{layer_idx}: routed down format metadata is missing")
    })?;
    let route_scale = hipfire_config::developer_var("HIPFIRE_DEEPSEEK4_ROUTE_SCALE")
        .ok()
        .and_then(|value| value.parse().ok())
        .unwrap_or(cfg.routed_scaling_factor);
    let params = hipfire_dispatch::families::moe::MoeBiasAwareParams {
        hidden: cfg.hidden_size,
        ...
        gate_up_dtype,
        down_dtype,
        x_rot: state.ffn_x_rot.as_ref().unwrap(),
        x_a8: if gate_up_dtype == DType::MQ2G256Lloyd
            && rdna_compute::mq2_i8dot::layer_enabled(Some(layer_idx))
        {
            state.moe_x_a8.as_ref()
        } else {
            None
        },
        ffn_out: routed_out,
        scores: state.router_scores.as_ref().unwrap(),
        gate_bias: layer.gate_bias.as_ref().unwrap(),
        expert_gate_up_ptrs: layer.expert_gate_up_ptrs.as_ref().unwrap(),
        expert_down_ptrs: layer.expert_w2_ptrs.as_ref().unwrap(),
        expert_owned_mask: layer.expert_owned_mask.as_ref(),
    ...
    hipfire_dispatch::pipeline::run_moe_decode_bias_aware_rows(gpu, &params, row_base, row_count)
        .map_err(|error| format!("ffn_routed_rows l{layer_idx}: {error}"))
}
fn ffn_hash_routed(
    cfg: &DeepseekV4Config,
    weights: &DeepseekV4Weights,
    ...
    let gate_up_dtype = layer.expert_gate_up_dtype.ok_or_else(|| {
        format!("ffn_hash_routed l{layer_idx}: routed gate/up format metadata is missing")
    })?;
    let down_dtype = layer.expert_down_dtype.ok_or_else(|| {
        format!("ffn_hash_routed l{layer_idx}: routed down format metadata is missing")
    })?;
    if layer.tid2eid_host.is_empty() {
        return Err(format!(
            "deepseek4: layer {layer_idx} is hash-routed but tid2eid is missing; \
             refusing the incorrect shared-only fallback (re-quantize the model)"
        ));
    }
    ...
    let route_scale: f32 = hipfire_config::developer_var("HIPFIRE_DEEPSEEK4_ROUTE_SCALE")
        ...
        .unwrap_or(cfg.routed_scaling_factor);
    let k_top = k;
    let probe_position = state.n_tokens;
    ...
    if let Some(tid2eid_dev) = layer.tid2eid_dev.as_ref() {
        ...
        scratch
            .w_scaled
            .extend(wts.iter().map(|&w| w * route_scale));
        scratch
            .w_bytes
    ...
    if let Some(expert_owned) = layer.expert_owned_mask.as_ref() {
        // The MQ4 wave64 indexed kernels do not accept negative expert ids.
        // Preserve the global ids and let the loader's zero gate/up dummy
        // pointer null the remote activation; MQ2/MQ3 retain their -1 skip.
        if gate_up_dtype != DType::MQ4G256 || down_dtype != DType::MQ4G256 {
            gpu.deepseek4_mask_topk_owned(topk_idx_dev, expert_owned, k_top, n_exp)
                .map_err(|e| format!("mask EP hash routes l{layer_idx}: {e:?}"))?;
        }
    }
    ...
    ep_trace_moe_f32(gpu, topk_w_dev, probe_position, layer_idx, "topk_weights")?;
    ep_trace_moe_f32(gpu, ffn_x_expert, probe_position, layer_idx, "expert_input")?;
    ...
    if i8dot && state.moe_x_a8.is_none() {
        ...
        state.moe_x_a8 = Some(
            gpu.alloc_tensor(&[bytes], DType::Raw)
                .map_err(|e| format!("alloc moe_x_a8 hash: {e:?}"))?,
        );
    }
    match gate_up_dtype {
        DType::HFP4G32 => {
            ...
            .map_err(|e| format!("HFP4 gate_up hash l{layer_idx}: {e:?}"))?;
        }
        DType::MQ4G256 => {
            ...
            .map_err(|e| format!("MQ4 gate_up hash l{layer_idx}: {e:?}"))?;
        }
        DType::MQ2G256Lloyd if i8dot => {
            let x_a8 = state.moe_x_a8.as_ref().unwrap();
            gpu.deepseek4_quantize_f32_a8_g128_gfx90a(ffn_x_rot, x_a8, cfg.hidden_size, true)
                .map_err(|e| format!("quantize gate_up hash l{layer_idx}: {e:?}"))?;
            ...
            .map_err(|e| format!("I8DOT gate_up hash l{layer_idx}: {e:?}"))?;
        }
        DType::MQ2G256Lloyd => {
            ...
            .map_err(|e| format!("MQ2 gate_up hash l{layer_idx}: {e:?}"))?;
        }
        DType::MQ3G256Lloyd => {
            ...
            .map_err(|e| format!("MQ3 gate_up hash l{layer_idx}: {e:?}"))?;
        }
        dtype => {
            return Err(format!(
                "ffn_hash_routed l{layer_idx}: unsupported gate/up dtype {dtype:?}"
            ));
        }
    }
    ...
    ep_trace_moe_f32(gpu, up_batch, probe_position, layer_idx, "up_pre_swiglu")?;
    ...
    ep_trace_moe_f32(gpu, gate_batch, probe_position, layer_idx, "post_swiglu")?;
    if down_dtype != DType::HFP4G32 {
    ...
    match down_dtype {
        DType::HFP4G32 => {
            ...
            .map_err(|e| format!("HFP4 down hash l{layer_idx}: {e:?}"))?;
        }
        DType::MQ4G256 => {
            ...
            .map_err(|e| format!("MQ4 down hash l{layer_idx}: {e:?}"))?;
        }
        DType::MQ2G256Lloyd => {
            ...
            .map_err(|e| format!("MQ2 down hash l{layer_idx}: {e:?}"))?;
        }
        DType::MQ3G256Lloyd => {
            ...
            .map_err(|e| format!("MQ3 down hash l{layer_idx}: {e:?}"))?;
        }
        dtype => {
            return Err(format!(
                "ffn_hash_routed l{layer_idx}: unsupported down dtype {dtype:?}"
            ));
        }
    }
...
fn attn_stub(
    ...
    ds4_fine_probe_end(gpu, attention_probe, layer_idx, "attention_swa")?;
    ...
    // Per-group sub-view offset depends on wo_a's dtype:
    //   MQ4G256 (Raw):      136 bytes per 256 elements
    //   Q8_0:                34 bytes per 32 elements
    //   F16/F32:             pass element counts; sub_offset multiplies by
    //                        dtype.size() internally.
    ...
    if tp_slice.is_some() {
        ...
        .map_err(|e| format!("gfx90a wave64 wo_a l{layer_idx}: {e:?}"))?;
    } else {
        for g in 0..n_groups {
            ...
            let wo_a_view = match wo_a.dtype {
                DType::F16 | DType::F32 => {
                    // Native floating source weights are contiguous by group.
                    // sub_offset handles F16/F32 byte scaling. The previous
                    // F16 fallthrough used the MQ4 packed-byte stride, so only
                    // group 0 read the correct weights and groups 1..7 silently
                    // consumed misaligned data.
...
fn attn_wo_b_project(
    ...
    if wo_b.dtype == DType::Q8_0 && ds4_gfx90a_wo_wave64_enabled(gpu) && wo_a.dtype == DType::Q8_0 {
        gpu.wo_per_group_batched_q8_0_wave64_row2_gfx90a(
            ...
            1,
            cfg.hidden_size as i32,
            groups_o_lora as i32,
            1,
        )
        .map_err(|e| format!("gfx90a wave64 wo_b l{layer_idx}: {e:?}"))?;
    } else {
        gemv_auto(
            gpu,
            wo_b,
...
fn hc_dump_here(state: &DeepseekV4State, gpu: &Gpu, layer_idx: usize, is_attn: bool) -> bool {
    if gpu.device_id != 0 || !is_attn {
        return false;
    }
    let layer_hit = hipfire_config::developer_var("HIPFIRE_DS4_DUMP_HC_LAYER")
        .ok()
        .and_then(|value| value.parse::<usize>().ok())
        == Some(layer_idx);
    let position_hit = hipfire_config::developer_var("HIPFIRE_DS4_DUMP_HC_POS")
        .ok()
        .and_then(|value| value.parse::<u64>().ok())
        == Some(state.n_tokens);
    layer_hit && position_hit
}
...
fn mhc_pre(
    ...
    let streams = state.residual_streams.as_ref().unwrap();
    let dump_hc = hc_dump_here(state, gpu, layer_idx, is_attn);
    let dump_prefix = format!(
        "hc_dev{}_pos{}_l{}_{}",
        gpu.device_id,
        state.n_tokens,
        layer_idx,
        if is_attn { "attn" } else { "ffn" },
    );
    if dump_hc {
        dump_buf(gpu, &format!("{dump_prefix}_streams_in"), streams);
    }
    ...
    if dump_hc {
        dump_buf(gpu, &format!("{dump_prefix}_control_with_base"), &c_view);
    }
    ...
    if dump_hc {
        dump_buf(gpu, &format!("{dump_prefix}_control_affine"), &c_view);
    }
    ...
    // but applied uniformly across all 4 streams; scalar decode and batched
    // prefill must use the same equation because a per-layer mismatch can
    // accumulate across the model.
    ...
    //
    // Upstream DeepSeek V4 defines post = 2 * sigmoid(...). Keep the
    // developer override for controlled experiments, but the default must
    // preserve the released model's residual-mixing equation.
    use std::sync::OnceLock;
    ...
    if dump_hc {
        dump_buf(gpu, &format!("{dump_prefix}_x_in"), hc_x_in);
    }
...
fn hc_attn_mix(
    ...
    let dump_hc = hc_dump_here(state, gpu, layer_idx, /*is_attn=*/ true);
    let dump_prefix = format!(
        "hc_dev{}_pos{}_l{}_attn",
        gpu.device_id, state.n_tokens, layer_idx,
    );
    if dump_hc {
        if let Some(tensor) = state.q_lat.as_ref() {
            dump_buf(gpu, &format!("{dump_prefix}_q_lat"), tensor);
        }
        if let Some(tensor) = state.q.as_ref() {
            dump_buf(gpu, &format!("{dump_prefix}_q_after_rope"), tensor);
        }
        if let Some(tensor) = state.kv.as_ref() {
            dump_buf(gpu, &format!("{dump_prefix}_kv_after_rope"), tensor);
        }
        if let Some(tensor) = state.attn_out_raw.as_ref() {
            dump_buf(gpu, &format!("{dump_prefix}_attn_derope"), tensor);
        }
        if let Some(tensor) = state.wo_a_out.as_ref() {
            dump_buf(gpu, &format!("{dump_prefix}_wo_a_out"), tensor);
        }
        dump_buf(gpu, &format!("{dump_prefix}_transform_out"), attn_out);
    }
    ...
    if dump_hc {
        dump_buf(gpu, &format!("{dump_prefix}_streams_out"), streams_out);
    }
...
fn q_lora(
    ...
    if ds4_fine_probe_enabled() && layer_idx == 0 {
        ...
        if !PRINTED.swap(true, std::sync::atomic::Ordering::Relaxed) {
            eprintln!(
                "DS4-FINE-CONFIG rank={} wq_a={:?} wq_b={:?}",
                gpu.device_id, wq_a.dtype, wq_b.dtype
            );
        }
    }
    let q_a_probe = ds4_fine_probe_start(gpu, layer_idx, state.n_tokens)?;
    if wq_a_needs_fwht {
        gpu.fused_rmsnorm_rotate_mq_plain(
            hc_x_in,
            attn_norm,
    ...
    ds4_fine_probe_end(gpu, q_a_probe, layer_idx, "q_lora_a")?;
    ...
    let q_b_probe = ds4_fine_probe_start(gpu, layer_idx, state.n_tokens)?;
...
pub(crate) fn precompute_attn_state_batched(
    ...
    for b in 0..batch_size {
        ...
        let values = attn_state_values(win, topk, max_compressed, pos);
        host[stripe..stripe + slots_per_pos].copy_from_slice(&values);
    }
...
fn init_residual_streams(
    ...
    // Dequant + lookup token row → embed_scratch [hidden]. Legacy HFQ
    // DeepSeek-V4 containers store Q8_0 embedding bytes in a Raw tensor; the
    // official 0731 safetensors loader normalizes BF16/F16 to native F16 and
    // tags it accordingly. Never feed those half bytes to the Q8 decoder.
    // Re-stage the id here for the F16 device-buffer kernel: graph decode runs
    // embedding outside capture, before the captured token-id memcpy.
    if token_embd.dtype == DType::F16 {
        precompute_token_id(state, gpu, token_id)?;
    }
    let embed_scratch = state.embed_scratch.as_ref().unwrap();
    if token_embd.dtype == DType::F16 {
        let token_id_buf = state
            .token_id_buf
            .as_ref()
            .ok_or_else(|| "init_residual_streams: token_id_buf not staged".to_string())?;
        gpu.embedding_lookup_f16_batched(token_embd, embed_scratch, token_id_buf, 1, hidden)
            .map_err(|e| format!("embedding_lookup_f16: {e:?}"))?;
    } else {
...
pub struct PrefillBatchScratch {
    ...
    pub moe_x_a8: GpuTensor,               // one SG8 A8 token, reused serially by I8DOT prefill
...
impl PrefillBatchScratch {
    ...
    pub fn new(gpu: &mut Gpu, cfg: &DeepseekV4Config, max_batch: usize) -> Result<Self, String> {
        ...
        Ok(Self {
            ...
            moe_x_a8: gpu
                .alloc_tensor(&[max_batch * (hidden / 128) * 136], DType::Raw)
                .map_err(|e| format!("PrefillBatchScratch alloc moe_x_a8: {e:?}"))?,
...
fn attention_block_batched_mixed(
    ...
    if ratio == 4 {
        ...
        if n_max_chunk == 0 {
            ...
            // Indexer Q shares the compressed layer's YaRN frequencies with
            // the main attention and Indexer compressor K.
            let (freq_base, freq_scale, ext_factor, attn_factor, corr_low, corr_high) =
                layer_rope_params(cfg, layer.compress_ratio);
            gpu.rope_tail_yarn_interleaved_batched(
                &pbs.idx_q_batch,
            ...
            .map_err(|e| format!("rope_tail_yarn_batched idx l{layer_idx}: {e:?}"))?;
...
/// quant time per the load_weights logic; missing tables are rejected.
#[allow(dead_code)]
fn ffn_batched(
    cfg: &DeepseekV4Config,
    ...
    let gate_up_dtype = layer.expert_gate_up_dtype.ok_or_else(|| {
        format!("ffn_batched l{layer_idx}: routed gate/up format metadata is missing")
    })?;
    let down_dtype = layer.expert_down_dtype.ok_or_else(|| {
        format!("ffn_batched l{layer_idx}: routed down format metadata is missing")
    })?;
    ...
    if hash_routing && layer.tid2eid_host.is_empty() {
        return Err(format!(
            "deepseek4: layer {layer_idx} is hash-routed but tid2eid is missing; \
             refusing the incorrect shared-only prefill fallback (re-quantize the model)"
        ));
    }
    ...
    if layer_idx == cfg.num_hash_layers {
        dump_buf(
            gpu,
            "router_first_score_layer_scores",
            &pbs.moe_scores_batch,
        );
    }
    ...
    let moe_params = hipfire_dispatch::families::moe::MoeBiasAwarePrefillParams {
        ...
        ffn_out: routed_out.unwrap_or(&pbs.ffn_out_batch),
...
///   pbs.hc_pre_batch[b, :]  = sigmoid(c[b, 0..4]) + hc_eps
...
/// `HIPFIRE_DEEPSEEK4_POST_SCALE` env override (default 2.0) is honoured.
#[allow(dead_code)]
fn mhc_pre_batched(
    cfg: &DeepseekV4Config,
    layer: &crate::deepseek4::DeepseekV4LayerWeights,
    ...
    //    with sigmoid+hc_eps on pre, post_scale·sigmoid on post.
...
pub fn forward_prefill_batch_chunk(
    ...
    .map_err(|e| format!("embedding_lookup_batched: {e:?}"))?;
...
/// Run one batched prefill chunk under expert parallelism.
///
/// Attention and the shared expert are replicated. Routed experts write a
/// rank-local batch-by-hidden partial; only that partial is all-reduced before
/// it is added to the replicated shared output and consumed by the HC FFN mix.
#[allow(clippy::too_many_arguments)]
pub fn forward_prefill_batch_chunk_ep(
    gpus: &mut hipfire_runtime::multi_gpu::Gpus,
    weights_per_rank: &[DeepseekV4Weights],
    cfg: &DeepseekV4Config,
    state_per_rank: &mut [DeepseekV4State],
    ...
    let ranks = gpus.devices.len();
    let n = tokens.len();
    if n == 0 {
        return Err("forward_prefill_batch_chunk_ep: empty tokens".to_string());
    }
    if weights_per_rank.len() != ranks
        || state_per_rank.len() != ranks
        || pbs_per_rank.len() != ranks
        || partials.len() != ranks
    {
        return Err("forward_prefill_batch_chunk_ep: rank vector length mismatch".to_string());
    }
    if pbs_per_rank.iter().any(|pbs| n > pbs.max_batch) {
        return Err(format!(
            "forward_prefill_batch_chunk_ep: chunk size {n} exceeds rank scratch"
        ));
    }

    hipfire_runtime::ep::ensure_rank_streams(gpus)
        .map_err(|e| format!("prefill EP streams: {e:?}"))?;
    ...
    let positions_bytes: &[u8] =
        unsafe { std::slice::from_raw_parts(positions_host.as_ptr() as *const u8, n * 4) };
    let n_valid_host: Vec<i32> = (0..n)
        .map(|b| ((start_pos as usize + b + 1).min(cfg.sliding_window)) as i32)
        .collect();
    let n_valid_bytes: &[u8] =
        unsafe { std::slice::from_raw_parts(n_valid_host.as_ptr() as *const u8, n * 4) };
    ...
    for r in 0..ranks {
        ...
        gpu.bind_thread()
            .map_err(|e| format!("prefill EP bind {r}: {e:?}"))?;
        gpu.memcpy_htod_auto(&pbs.tokens.buf, token_bytes)
            .map_err(|e| format!("prefill EP tokens r{r}: {e:?}"))?;
        gpu.memcpy_htod_auto(&pbs.positions.buf, positions_bytes)
            .map_err(|e| format!("prefill EP positions r{r}: {e:?}"))?;
        if r == 0 {
            dump_buf(gpu, "ep_00_tokens", &pbs.tokens);
            dump_buf(gpu, "ep_00_positions", &pbs.positions);
        }
        gpu.memcpy_htod_auto(&pbs.n_valid_swa_arr.buf, n_valid_bytes)
            .map_err(|e| format!("prefill EP n_valid r{r}: {e:?}"))?;
        let token_embd = weights_per_rank[r]
            .token_embd
            .as_ref()
            .ok_or_else(|| format!("prefill EP token_embd missing r{r}"))?;
        match token_embd.dtype {
            DType::F16 => gpu.embedding_lookup_f16_batched(
                token_embd,
                &pbs.embed_batch,
        ...
        .map_err(|e| format!("prefill EP embedding r{r}: {e:?}"))?;
        if r == 0 {
            dump_buf(gpu, "ep_01_embed", &pbs.embed_batch);
        }
        gpu.hc_streams_init_from_embed_batched(
            &pbs.embed_batch,
            &pbs.streams_batch,
        ...
        .map_err(|e| format!("prefill EP stream init r{r}: {e:?}"))?;
        if r == 0 {
            dump_buf(gpu, "ep_02_streams_init", &pbs.streams_batch);
        }
    }

    let partial_count = n * cfg.hidden_size;
    for layer_idx in 0..cfg.num_hidden_layers {
        for r in 0..ranks {
            let gpu = &mut gpus.devices[r];
            let weights = &weights_per_rank[r];
            ...
            if r == 0 && layer_idx == 0 {
                dump_buf(gpu, "ep_l0_attn_streams_in", &pbs.streams_batch);
            }
            ...
            if r == 0 && layer_idx == 0 {
                dump_buf(gpu, "ep_03_l0_mhc_attn", &pbs.hc_x_in_batch);
            }
            ...
            if r == 0 && layer_idx == 0 {
                dump_buf(gpu, "ep_04_l0_q", &pbs.q_batch);
            }
            ...
            if r == 0 && layer_idx == 0 {
                dump_buf(gpu, "ep_05_l0_q_rope", &pbs.q_batch);
                dump_buf(gpu, "ep_06_l0_kv_rope", &pbs.kv_batch);
            }
            ...
            if r == 0 && layer_idx == 0 {
                dump_buf(gpu, "ep_07_l0_attn_out", &pbs.attn_out_batch);
            }
            ...
            if r == 0 && layer_idx == 0 {
                dump_buf(gpu, "ep_08_l0_attn_streams", &pbs.streams_batch);
            }

            if r == 0 && layer_idx == 0 {
                dump_buf(gpu, "ep_l0_ffn_streams_in", &pbs.streams_batch);
            }
            let probe = ds4_coarse_probe_start(gpu, layer_idx, probe_step)?;
            mhc_pre_batched(cfg, layer, pbs, gpu, layer_idx, false, n)?;
            ds4_coarse_probe_end(gpu, probe, layer_idx, "batch_mhc_ffn_pre")?;
            if r == 0 && layer_idx == 0 {
                dump_buf(gpu, "ep_09_l0_mhc_ffn", &pbs.hc_x_in_batch);
            }
            ...
            let stream = gpu
                .active_stream
                .as_ref()
                .ok_or_else(|| format!("prefill EP active stream missing r{r}"))?;
            gpu.hip
                .memset_async(&partials[r].buf, 0, partial_count * 4, stream)
                .map_err(|e| format!("prefill EP zero partial r{r}: {e:?}"))?;
            ffn_batched(
                cfg,
                layer,
                pbs,
            ...
            if r == 0 && layer_idx == 0 {
                dump_buf(gpu, "ep_10_l0_shared", &pbs.ffn_out_batch);
                dump_buf(gpu, "ep_11_l0_routed_local", &partials[r]);
            }
        }

        let refs: Vec<&hip_bridge::DeviceBuffer> =
            partials.iter().map(|partial| &partial.buf).collect();
        gpus.all_reduce_sum_f32_peer(&refs, partial_count)
            .map_err(|e| format!("prefill EP all-reduce l{layer_idx}: {e:?}"))?;

        for r in 0..ranks {
            let gpu = &mut gpus.devices[r];
            let pbs = &pbs_per_rank[r];
            gpu.bind_thread()
                .map_err(|e| format!("prefill EP post-AR bind r{r}: {e:?}"))?;
            let out = pbs.ffn_out_batch.sub_offset(0, partial_count);
            let partial = partials[r].sub_offset(0, partial_count);
            if r == 0 && layer_idx == 0 {
                dump_buf(gpu, "ep_12_l0_routed_reduced", &partial);
            }
            ...
            if r == 0 && layer_idx <= 3 {
                dump_buf(
                    gpu,
                    &format!("ep_13_l{layer_idx}_end_streams"),
                    &pbs.streams_batch,
                );
            }
        }
    ...
    // Match the scalar forward and non-EP batched-prefill state contract:
    // the next decode reads state.n_tokens to derive its SWA ring slot,
    // valid-window count, and compressor/indexer state. Leaving this at the
    // pre-chunk value makes the prefill-final logits correct but corrupts the
    // very next token's attention state.
    for state in state_per_rank.iter_mut() {
        state.n_tokens = start_pos as u64 + n as u64;
    }

    gpus.devices[0]
        .bind_thread()
        .map_err(|e| format!("prefill EP final bind: {e:?}"))?;
    let result = final_norm_and_head_last_batched(
        cfg,
        &weights_per_rank[0],
        &mut state_per_rank[0],
        &pbs_per_rank[0],
    ...
    if result.is_ok() {
        if let Some(final_norm) = state_per_rank[0].final_norm.as_ref() {
            dump_buf(&mut gpus.devices[0], "ep_14_final_norm", final_norm);
        }
        if let Some(logits) = state_per_rank[0].logits.as_ref() {
            dump_buf(&mut gpus.devices[0], "ep_15_logits", logits);
        }
    }
    result
}
...
pub fn forward_prefill_batch_chunked(
    ...
    while !remaining.is_empty() {
        ...
        // The batched chunk mutates the SWA/compressor/indexer caches but does
        // not own the public sequence cursor.  Advance it before returning the
        // final prefill logits so the immediately following decode observes
        // the same frontier as the EP and MTP prefill paths.
        state.n_tokens = (pos_cursor + take) as u64;
...
mod tests {
    ...
    #[test]
    fn compressed_attention_state_clamps_at_cache_capacity() {
        let at_last_valid_commit = attn_state_values(128, 512, 2048, 8191);
        assert_eq!(at_last_valid_commit[2], 2048);
        assert_eq!(at_last_valid_commit[4], 512);
        assert_eq!(at_last_valid_commit[7], 2047);

        let just_past_capacity = attn_state_values(128, 512, 2048, 8192);
        assert_eq!(just_past_capacity[2], 2048);
        assert_eq!(just_past_capacity[4], 512);
        assert_eq!(just_past_capacity[7], -1);

        let first_oob_commit_without_clamp = attn_state_values(128, 512, 2048, 8195);
        assert_eq!(first_oob_commit_without_clamp[2], 2048);
        assert_eq!(first_oob_commit_without_clamp[4], 512);
        assert_eq!(first_oob_commit_without_clamp[7], -1);
    }

    #[test]
    fn ratio128_attention_state_clamps_at_cache_capacity() {
        let at_last_valid_commit = attn_state_values(128, 512, 2048, 262_143);
        assert_eq!(at_last_valid_commit[3], 2048);
        assert_eq!(at_last_valid_commit[5], 512);
        assert_eq!(at_last_valid_commit[9], 2047);

        let first_oob_commit_without_clamp = attn_state_values(128, 512, 2048, 262_271);
        assert_eq!(first_oob_commit_without_clamp[3], 2048);
        assert_eq!(first_oob_commit_without_clamp[5], 512);
        assert_eq!(first_oob_commit_without_clamp[9], -1);
    }

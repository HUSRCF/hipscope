...
/// dense HFQ4 down projection. Returns false without side effects for every
/// unsupported dtype, shape, architecture, or AWQ-scaled weight.
#[inline]
        ...
        match (&weights.layers[layer_idx], config.layer_types[layer_idx]) {
            (LayerWeights::DeltaNet(layer), LayerType::LinearAttention) => {
                ...
                if direct_group128_down {
                    // The fused producer and prequantized residual GEMM above
                    // completed the entire down-projection stage.

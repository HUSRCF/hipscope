...
fn main() -> HipResult<()> {
    ...
    eprintln!(
        ...
        trunk_config.dim, trunk_config.n_layers, trunk_config.vocab_size, trunk_config.eos_token,
    );
    ...
    let mut trunk_dn = DeltaNetState::new(&mut gpu, &trunk_config).expect("DeltaNetState::new");
    let trunk_scratch =
        Qwen35Scratch::new(&mut gpu, &trunk_config, 64).expect("Qwen35Scratch::new");
    ...
    let prev_hidden_host = gpu
        .download_f32(&trunk_scratch.tmp)
    ...
    let mtp_scratch =
        mtp_head::Qwen35MtpHeadScratch::new(&mut gpu, &head.config).expect("MtpHeadScratch::new");
    let mut mtp_kv =
        mtp_head::Qwen35MtpHeadKvCache::new(&mut gpu, &head.config).expect("MtpHeadKvCache::new");

...
fn run_model(path: &str, args: &Args) -> Vec<Vec<f32>> {
    ...
    assert_eq!(hfq.arch_id, 12, "kld_logits(cohere2moe): expected arch_id 12, got {}", hfq.arch_id);
...
fn print_summary(kls: &[f64]) {
    ...
    println!("frac>0.1  : {:.4}", kls.iter().filter(|&&k| k > 0.1).count() as f64 / n as f64);
}
...
fn main() {
    ...
    let mb = args.model_b.clone().expect("--model-b required (or use --dump)");

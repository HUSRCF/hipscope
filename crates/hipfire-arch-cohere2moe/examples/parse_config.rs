...
fn main() {
    ...
    println!("{cfg:#?}");
    let n_full = cfg.layer_types.iter().filter(|&&k| k == AttnKind::Full).count();
    let n_slide = cfg.num_hidden_layers - n_full;
    println!("---");
    println!("q_dim={} kv_dim={}", cfg.q_dim(), cfg.kv_dim());
    println!("full(NoPE)={n_full}  sliding(RoPE)={n_slide}  dense_prefix={}", cfg.first_k_dense_replace);
    println!("layer0 dense={}  ffn_inter(L0)={}  ffn_inter(L1)={}",
        cfg.is_dense_ffn(0), cfg.ffn_intermediate(0), cfg.ffn_intermediate(1));
    println!("uses_rope: L0={} L1={} L4={}", cfg.uses_rope(0), cfg.uses_rope(1), cfg.uses_rope(4));
}

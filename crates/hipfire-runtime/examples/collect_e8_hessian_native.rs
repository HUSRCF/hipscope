...
fn main() {
    ...
    while i < argv.len() {
        match argv[i].as_str() {
            ...
            "--slice" => { slices.push(PathBuf::from(&argv[i + 1])); i += 2; }
    ...
    assert!(!slices.is_empty(), "at least one --slice <calib.txt> required");
    ...
    eprintln!(
        ...
        gpu.arch, model.display(), config.num_experts, config.num_experts_per_tok,
    ...
    eprintln!("hipfire tokenize: {} tokens from {} slice(s)", tokens.len(), slices.len());
    ...
    for c in 0..n_chunk {
        ...
        for pos in 0..(n_ctx - 1) {
            ...
            if steps % 64 == 0 {
                ...
                let n_ent = gpu.hessian_capture.as_ref().map(|c| c.entries.len()).unwrap_or(0);
                eprint!(
                    "\r  chunk {:4}/{}  step {:7}  entries {:6}  ({:.1} tok/s)   ",
                    c + 1, n_chunk, steps, n_ent, steps as f64 / el.max(1e-9)
                );
            }
        }
    }
    ...
    for k in ex.iter().take(3) {
        ...
        eprintln!("  e.g. {k}  (K={}, n_blocks={}, rows={}, mean_diag={:.4e})",
            acc.k, acc.n_blocks, acc.n_rows, acc.mean_diag());
    }
}

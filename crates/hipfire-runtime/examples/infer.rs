...
fn main() {
    ...
    if vl_mode {
        eprintln!("Image: {}", image_path.as_ref().unwrap());
    }
    ...
    eprintln!(
        "Text: dim={}, layers={}, vocab={}",
        text_config.dim, text_config.n_layers, text_config.vocab_size
    );
    ...
    if vl_mode {
        let vision_config =
            qwen35_vl::vision_config_from_hfq(&hfq).expect("no vision config in model");
        eprintln!(
            "Vision: hidden={}, layers={}, heads={}",
            vision_config.hidden_size, vision_config.num_layers, vision_config.num_heads
        );
        ...
        n_visual_tokens = (grid_h * grid_w)
            / (vision_config.spatial_merge_size * vision_config.spatial_merge_size);
        ...
        if let Ok(dump_dir) = std::env::var("HIPFIRE_VL_DUMP_DIR") {
            ...
            let elems = vision_config.temporal_patch_size
                * 3
                * vision_config.patch_size
                * vision_config.patch_size;
            ...
            eprintln!(
                "[VL-DUMP] wrote CHW ({}x{}x{} f32) + patches ({}x{} f32) to {}",
    ...
    if debug_cmp {
        ...
        eprintln!(
            "=== Debug: comparing {} tokens through both paths ===",
            test_seq.len()
        );
        for (i, &tok) in test_seq.iter().enumerate() {
            ...
            let mut sa: Vec<(usize, f32)> =
                logits_a.iter().enumerate().map(|(i, &v)| (i, v)).collect();
            sa.sort_by(|a, b| b.1.partial_cmp(&a.1).unwrap());
            let mut sb: Vec<(usize, f32)> =
                logits_b.iter().enumerate().map(|(i, &v)| (i, v)).collect();
    ...
    eprintln!(
        ...
        if vl_mode {
            format!(
                " ({} visual + {} text)",
                n_visual_tokens,
                prompt_tokens.len() - n_visual_tokens
            )
        } else {
            String::new()
        }
    ...
    for _ in 0..max_gen {
        ...
        let think_ended = in_thinking
            && (think_count >= max_think || {
                ...
                gl >= think_end_seq.len()
                    && generated[gl - think_end_seq.len()..] == think_end_seq[..]
            });
        ...
        if next_token == text_config.eos_token {
            break;
        }
...
fn debug_compare(
    ...
    // Path A: forward()
    let logits_a =
        hipfire_arch_qwen35::qwen35::forward(gpu, weights, config, token, 0, kv_cache1, dn_state1)
            .unwrap();

    // Path B: forward_scratch()
    hipfire_arch_qwen35::qwen35::forward_scratch(
        gpu, weights, config, token, 0, kv_cache2, dn_state2, scratch,
    )
    .unwrap();
    let logits_b = gpu.download_f32(&scratch.logits).unwrap();
    ...
    eprintln!(
        "COMPARE: max_diff={max_diff:.6} at idx={max_idx}, n_diff(>0.001)={n_diff}/{}",
        logits_a.len()
    );
    eprintln!(
        "  A[{max_idx}]={:.6}  B[{max_idx}]={:.6}",
        logits_a[max_idx], logits_b[max_idx]
    );
    // Check top-5
    let mut sorted_a: Vec<(usize, f32)> =
        logits_a.iter().enumerate().map(|(i, &v)| (i, v)).collect();
    sorted_a.sort_by(|a, b| b.1.partial_cmp(&a.1).unwrap());
    let mut sorted_b: Vec<(usize, f32)> =
        logits_b.iter().enumerate().map(|(i, &v)| (i, v)).collect();

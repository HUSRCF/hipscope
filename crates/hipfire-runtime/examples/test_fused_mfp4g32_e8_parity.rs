...
fn main() {
    ...
    let layer = weights.layers.iter().find_map(|l| match l {
    ...
    // ── gate+up parity ──
    assert_eq!(layer.w_gate.gpu_dtype, DType::MFP4G32E8, "gate must be MFP4G32E8");
    assert_eq!(layer.w_up.gpu_dtype, DType::MFP4G32E8, "up must be MFP4G32E8");
    {
        ...
        gpu.fused_gate_up_mfp4g32_e8(&layer.w_gate.buf, &layer.w_up.buf, &x_dev,
            &yfg, &yfu, gm, um, k).expect("fused gate_up");
        let yrg = gpu.alloc_tensor(&[gm], DType::F32).unwrap();
        let yru = gpu.alloc_tensor(&[um], DType::F32).unwrap();
        gpu.gemv_mfp4g32_e8(&layer.w_gate.buf, &x_dev, &yrg, gm, k).expect("gemv gate");
        gpu.gemv_mfp4g32_e8(&layer.w_up.buf, &x_dev, &yru, um, k).expect("gemv up");
        fails += cmp("gate", &gpu, &yfg, &yrg);
        fails += cmp("up",   &gpu, &yfu, &yru);
    }

    // ── qkvza parity ──
    assert_eq!(layer.wqkv.gpu_dtype, DType::MFP4G32E8, "wqkv must be MFP4G32E8");
    ...
    if la4_e8 {
        ...
        gpu.fused_qkvza_mfp4g32_e8(&layer.wqkv.buf, &layer.wz.buf, &layer.w_beta.buf,
            &layer.w_alpha.buf, &x_dev, &yfq, &yfz, &yfb, &yfa, mqkv, mz, mb, ma, k)
            .expect("fused qkvza");
        ...
        gpu.gemv_mfp4g32_e8(&layer.wqkv.buf, &x_dev, &yrq, mqkv, k).unwrap();
        gpu.gemv_mfp4g32_e8(&layer.wz.buf, &x_dev, &yrz, mz, k).unwrap();
        gpu.gemv_mfp4g32_e8(&layer.w_beta.buf, &x_dev, &yrb, mb, k).unwrap();
        gpu.gemv_mfp4g32_e8(&layer.w_alpha.buf, &x_dev, &yra, ma, k).unwrap();
...
#[cfg(feature = "deltanet")]
fn cmp(name: &str, gpu: &rdna_compute::Gpu, fused: &rdna_compute::GpuTensor, refr: &rdna_compute::GpuTensor) -> u32 {
    ...
    if ndiff == 0 {
        ...
        eprintln!("  {name:6}: {ndiff}/{} rows differ (max |Δ|={max_abs:.3e}) FAIL", a.len());
        1
    }
}

//! dflash_mtp_tree_demo: DFlash + per-slot MTP tree composition bench (Task 11b).
//!
//! Mirrors `dflash_mtp_demo.rs` but uses `spec_step_dflash_mtp_tree` which
//! attaches K MTP children to EACH dflash slot (not just the last one), so an
//! early dflash miss still provides MTP candidates from the LAST accepted
//! dflash slot.
//!
//! Tree shape per cycle (B=16, K=2):
//!   1 root + 15 dflash + 30 MTP children = 46 nodes
//!
//! Greedy / temp=0 only.
//!
//! Usage:
...
fn main() {
    use hipfire_arch_qwen35::mtp_compose::{self, MtpComposeTreeState};
    use hipfire_arch_qwen35::mtp_head;
    use hipfire_arch_qwen35::speculative::{
        self, DdtreeScratch, DeltaNetSnapshot, GdnTape, HiddenStateRingBuffer, ModelSlot,
        ModelSlotConfig, VerifyScratch,
    };
    ...
    if temp != 0.0 {
        eprintln!("error: dflash_mtp_tree_demo v1 is greedy-only (--temp must be 0.0); got {temp}");
        std::process::exit(2);
    }
    ...
    eprintln!("=== dflash_mtp_tree_demo (per-slot tree, Task 11b) ===");
    ...
    if let Some(b) = dflash_b {
        ...
        if b != orig {
            eprintln!("block_size override: {orig} -> {b}");
    ...
    let n_verify = 1 + n_dflash + n_mtp_total; // tree size = 1 root + B-1 dflash + (B-1)*K MTP
    ...
    eprintln!(
        "tree: {} nodes (1 root + {} dflash + {} MTP children)",
        n_verify, n_dflash, n_mtp_total
    );

    // ── Load trunk ─────────────────────────────────────────────────────
    let mut slot_cfg = ModelSlotConfig::default();
    // Per cycle, position advances by accept + 1 ≤ B + K. Verify writes
    // n_verify KV slots before rollback so we need padding.
    ...
    assert_eq!(target.config.vocab_size, draft_cfg.vocab_size);
    assert_eq!(target.config.dim, draft_cfg.hidden);
    ...
    if chatml {
        ...
        assert!(im_start.len() == 1, "tokenizer has no <|im_start|>");
    ...
    assert!(prompt_tokens.len() + max_tokens + n_verify + 16 <= max_seq_total);

    // ── Hidden ring buffer + snapshot + verify scratch + ddtree scratch ─
    ...
    let mut full_dflash_cycles = 0usize;
    ...
    while !hit_eos && emitted.len() < max_tokens {
        ...
        let result = mtp_compose::spec_step_dflash_mtp_tree(
            &mut gpu,
            &mut target,
            &draft_weights,
            &draft_cfg,
        ...
        .expect("spec_step_dflash_mtp_tree");

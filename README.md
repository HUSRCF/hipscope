...
## Optional gfx11 CK prefill preview

The `master` branch includes an optional gfx11 CK prefill route together with
...
For a new source build from the current integration:
...
To use the frozen preview instead:
...
To migrate another development branch, merge the published snapshot
explicitly and resolve any local kernel or FeatureFlag changes:

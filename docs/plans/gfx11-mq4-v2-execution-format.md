...
| Concurrent gate/up on two HIP streams | 0.9850x per pair; bit-exact | closed |
| N2 fragment reuse plus quad-row weight staging | 1.0116x gate/set, 0.9973x down/add; bit-exact | closed |
...
The two independent gate/up projections also did not overlap profitably on
separate HIP streams. Their paired median changed from 10.3952 to 10.5532 ms,
with bit-exact outputs. Combining the exact N2 LDS-fragment reuse loop with
the retained quad-row weight loader was likewise too small: gate/set reached
1.0116x while down/add reached 0.9973x. The two-gate-plus-one-down weighted
speedup is only about 1.0068x, so neither path advanced to PP16384. The exact
record is in
`experiments/gfx11-mq4-v2/results/exact-structural-probes-gpu1-20260812/`.
